...
DEFAULT_YDL_SOCKET_TIMEOUT = 12
DEFAULT_YDL_RETRIES = 1
DEFAULT_YDL_EXTRACTOR_RETRIES = 1


class YouTubeURLValidator:
    @staticmethod
    def _apply_network_defaults(opts: Dict[str, Any]) -> Dict[str, Any]:
        opts.setdefault('socket_timeout', DEFAULT_YDL_SOCKET_TIMEOUT)
        opts.setdefault('retries', DEFAULT_YDL_RETRIES)
        opts.setdefault('extractor_retries', DEFAULT_YDL_EXTRACTOR_RETRIES)
        return opts
    ...
    def check_existence(video_id, extra_opts=None):
        ...
        try:
            ...
            ydl_opts = YouTubeURLValidator._apply_network_defaults(ydl_opts)
    ...
    def _build_ydl_opts(base_opts, extra_opts=None):
        ...
        YouTubeURLValidator._apply_network_defaults(opts)
...
def _single_media_opts(auth_opts: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    opts: Dict[str, Any] = {
        ...
        'skip_download': True,
        'noplaylist': True,
        'socket_timeout': DEFAULT_YDL_SOCKET_TIMEOUT,
        'retries': DEFAULT_YDL_RETRIES,
        'extractor_retries': DEFAULT_YDL_EXTRACTOR_RETRIES,
    }
    if auth_opts:
